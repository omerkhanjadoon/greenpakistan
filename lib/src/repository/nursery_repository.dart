...
Future<Stream<Nursery>> getTopNursery() async {
  ...
  return streamedRest.stream
      ...
      .map((data) {
    return Nursery.fromJSON(data);
  });
}

Future<Stream<Nursery>> getNearNursery(
...
Future<Stream<Nursery>> searchNursery(
...
Future<Stream<Nursery>> getNursery(String id) async {
  User _user = await getCurrentUser();
  final String _apiToken = 'api_token=${_user.apiToken}';
  final String url =
  ...
  return streamedRest.stream
      ...
      .transform(json.decoder)
      .map((data) => Helper.getData(data))
      .map((data) => Nursery.fromJSON(data));
}

Future<Stream<Review>> getNurseryReviews(String id) async {
  User _user = await getCurrentUser();
  final String _apiToken = 'api_token=${_user.apiToken}&';
  final String url =
